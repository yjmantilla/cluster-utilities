...
Everything here is reversible — undo whatever you no longer want:

- **SSH hardening (re-allow passwords):**
  `sudo rm /etc/ssh/sshd_config.d/00-harden.conf && sudo systemctl reload sshd`
- **Revoke a phone's key:** remove its line from `~/.ssh/authorized_keys` on the workstation
  (identify it by its comment, e.g. `Generated By Termius`).
- **Drop the shared master (force a fresh Duo next time):** `ssh -O exit fir` (and/or
  `trillium`, …).
- **`ControlPersist` / aliases:** edit `~/.ssh/config` — set `ControlPersist` back to a
  timeout (e.g. `4h`) or delete the `Host` alias blocks.
- **Tailscale:** `sudo tailscale down` (disconnect) · `sudo tailscale logout` (forget
  identity) · `sudo systemctl disable --now tailscaled && sudo pacman -Rns tailscale` (full
  removal). On the phone, delete the workstation from the Tailscale app or sign out.
- **SSH server (stop accepting connections):** `sudo systemctl disable --now sshd`.
- **Phone apps:** delete the Termius host + its generated key; uninstall Termius/Tailscale if
  unused elsewhere. The `bin/` helpers are just files in this repo (nothing was installed to
  `PATH`) — remove them by deleting the checkout.
